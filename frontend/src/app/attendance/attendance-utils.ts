...
/**
 * The club's operating time zone. Every schedule the backend stores is a
 * `dia_semana` + wall-clock `hora_inicio`/`hora_fin` with no offset attached,
 * so "which day is it" is a question about the club, not about the reader.
 *
 * Ecuador's mainland is a fixed UTC-5 with no DST, but the offset is NOT
 * hardcoded: an IANA zone keeps the intent readable and survives any future
 * rule change without a silent one-hour drift.
 */
 ...
 *
 * Prefer this over `jsDayIndexToDiaSemana(new Date().getDay())`: `getDay()`
 * reads the device's time zone, which is a fact about the reader's OS clock
 * and not about where the training happens. A laptop left on UTC — or on the
 * zone of last month's trip — sits two hours past Ecuador's midnight and
 * silently offers tomorrow's schedules, with data that looks entirely
 * plausible. The app being used only in Ecuador does not fix that: it
 * constrains where people stand, not how their clock is configured.
 *
 * `now` is injectable so callers (and tests) can pin the instant.
 */
/**
 * Built once and reused: callers resolve the day on every render, and
 * constructing a formatter per call would be the expensive part.
 */
...
export function todayDiaSemana(now: Date = new Date()): DiaSemana {
  try {
    ...
    // A runtime without the zone throws above rather than returning a bad
    // day; an unrecognized label can only mean a locale surprise, so fall
    // through to the device day rather than render a broken week.
    return EN_WEEKDAY_TO_DIA_SEMANA[weekday] ?? jsDayIndexToDiaSemana(now.getDay());
  } catch {
    // Degrades to the previous behaviour (device day) instead of blanking
    // the screen — a wrong day is recoverable by the "ver todos" escape
    // hatch, an exception in a `useMemo` is not.
    return jsDayIndexToDiaSemana(now.getDay());
  }
}
